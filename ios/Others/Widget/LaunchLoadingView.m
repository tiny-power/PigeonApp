...
- (void)startTimer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        self.timerLabel.text = [NSString stringWithFormat:@"%@ 00:00 ,%@:%d",@"当前",@"总超时时间",FirstAppOpen_Timeout];
    });
...
- (void)updateTimer {
    ...
    // 确保UI更新在主线程执行
    dispatch_async(dispatch_get_main_queue(), ^{
        self.timerLabel.text = [NSString stringWithFormat:@"%@ %02ld:%02ld ,%@:%d",@"当前" ,(long)minutes, (long)remainingSeconds,@"总超时时间",FirstAppOpen_Timeout];
    });
}
